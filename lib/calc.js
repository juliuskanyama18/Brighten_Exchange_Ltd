 ...
 * @param settings       Settings document (marginTlTsh, buyMarginPercent, deliveryFeeTl)
 ...
 */
export function calculateQuote({ direction, currency, amount, settings, rates, needsDelivery = false }) {
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new QuoteError('Invalid amount');
  }

  const { marginTlTsh, buyMarginPercent, deliveryFeeTl } = settings;
  ...
  if (direction === 'want_tsh') {
    ...
    if (!needsDelivery) {
      return { direction, currency, amount, ...result, needsDelivery: false };
    }
    ...
    return { direction, currency, amount, buyRate: result.buyRate, grossTsh, deliveryFeeTsh, finalTsh, needsDelivery: true };
  }
