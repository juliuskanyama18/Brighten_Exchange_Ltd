 ...
 *
 *  TL uses a FIXED TSh offset (settings.marginTlTsh) rather than a
 *  percentage — at TL's magnitude (~50-60 TSh) a percentage would need
 *  constant retuning, and the business thinks in flat TSh terms for TL.
 *  USD/EUR/GBP use percentages (settings.sellMarginPercent /
 *  settings.buyMarginPercent), since a flat TSh offset would be meaningless
 *  at their magnitude (~2,000-4,000 TSh/unit).
 *
...
export function getBuyRate(currency, rates, buyMarginPercent, marginTlTsh) {
  const reference = getReferenceRate(currency, rates);
  if (reference === null) return null;
  if (currency === 'TL') return reference - marginTlTsh;
 ...
 */
export function calculateForeignToTsh({ currency, amount, rates, buyMarginPercent, marginTlTsh }) {
  const buyRate = getBuyRate(currency, rates, buyMarginPercent, marginTlTsh);
...
export function calculateQuote({ direction, currency, amount, settings, rates }) {
  ...
  if (direction === 'want_tsh') {
    ...
    const result = calculateForeignToTsh({ currency, amount, rates, buyMarginPercent, marginTlTsh });
